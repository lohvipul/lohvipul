...
public UserDtls updateUserProfile(UserDtls user, MultipartFile img) {
    ...
    // Update profile image name if a new file is uploaded
    if (!img.isEmpty()) {
        dbUser.setProfileImage(img.getOriginalFilename());
    }
    ...
    // Save uploaded image to external folder
    try {
        if (!img.isEmpty()) {
            ...
            Path path = Paths.get(saveDir.getAbsolutePath(), img.getOriginalFilename());
            System.out.println("Updating profile image: " + path);
